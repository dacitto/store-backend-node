...
export default abstract class Repository<T> implements BaseRepository<T> {
  ...
  async listAsync(options?: {
    ...
    const orderByQuery = options?.orderBy
      ? `ORDER BY ${Object.keys(options.orderBy)[0]} ${
          Object.values(options.orderBy)[0] ?? 'ASC'
        }`
      : ''
    const filterQuery = options?.filter
      ? `WHERE ${Object.keys(options.filter)[0]} = '${
          Object.values(options.filter)[0]
        }'`
      : ''
    const paginateQuery = options?.paginate
      ? `LIMIT ${options.paginate.perPage ?? 10} OFFSET ${
          ((options.paginate.page ?? 1) - 1) * (options.paginate.perPage || 10)
        }`
      : ''
    ...
    return options?.paginate
      ? {
          ...
          pageSize: options.paginate.perPage ?? 10,
        }
      : result.rows
  }
